...
namespace object {
	...
	public:
            ...
            if(requester == nullptr || container_permissions_->canInsert(static_cast<T*>(this)->shared_from_this(), requester, object))
        	{
        	    ...
        	    if (arrangement_id < 4)
        	    {
        	    	...
        	    	if (removed_object)
        	    	{
                        ...
                        object->GetContainer()->AddObject(nullptr, removed_object, -2);
        	    	}
        	    }
        	    else
        	    {
            ...
            int32_t arrangement_id=-2)
        {
        	...
        	{
                newContainer->AddObject(nullptr, object, arrangement_id);
        	}
        }
        ...
        {
	        return GetObjects(nullptr, 1, true).size() > 0;
        }
        ...
        void SetContainer(const std::shared_ptr<T>& container)
        {
            container_ = container;
        }
        
        const std::shared_ptr<T>& GetContainer()
        {
        	return container_;
        }
        
        template<typename U>
        std::shared_ptr<U> GetContainer()
        {
#ifdef _DEBUG
            return std::dynamic_pointer_cast<U>(container_);
#else
            return std::static_pointer_cast<U>(container_);
#endif
        }
